      ...
      (set) => ({
        ...
        setResultPaint: (data) =>
          set((state) => {
            ...
            let materialName: string = selectShapes.find(
              (i) => i.value === material
            )?.label!;

            // Calculating
            const consume: number =
              ((weight * thick) / (efficiency / 100)) * layersData * numberData;
            const cover: number = 1000 / (weight * thick);
            const priceCover: number =
              ((weight * thick * price) / (efficiency * 10)) *
              layersData *
              numberData;
            const necessity: number =
              ((weight * square * thick) / 1000) *
              layersData *
              numberData *
              both;
        ...
        // removePaintResult
        removePaintResult: (id) =>
          set((state) => ({
            resultsPaint: state.resultsPaint.filter((el) => el.id !== id),
          })),
